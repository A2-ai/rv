...
use crate::configure::{
    ConfigureError, ConfigureErrorKind, ConfigureType, clear_array, format_array, get_mut_array,
    read_config_as_document, to_value_bool, to_value_string,
};
...
pub fn execute_repository_action(
    ...
    let (operation, response_alias, response_url, message) = match action {
        RepositoryAction::Clear => {
            clear_array(&mut doc, ConfigureType::Repository).map_err(|e| ConfigureError {
                path: config_file.into(),
                source: Box::new(e),
            })?;
            (
...
pub(super) fn find_repository_index(repos: &Array, alias: &str) -> Option<usize> {
...
fn create_repository_value(alias: &str, url: &Url, force_source: bool) -> Value {
    ...
    table.insert("alias", to_value_string(alias));
    table.insert("url", to_value_string(url));

    if force_source {
        table.insert("force_source", to_value_bool(true));
    }
