...
pub fn activate(dir: impl AsRef<Path>) -> Result<(), ActivateError>{
    ...
    if !dir.is_dir() {
        return Err(ActivateError{source: ActivateErrorKind::NotDir(dir.to_path_buf())})
    }
...
// Read the .Rprofile, if the file does not exist, create it
...
fn read_file(file_name: impl AsRef<Path>) -> Result<(String, File), ActivateError> {
    ...
    if !file_name.exists() {
        let file = File::create(file_name).map_err(|e| ActivateError { source: ActivateErrorKind::Io(e) })?;
        return Ok((String::new(), file))
    }
    let mut file = File::open(file_name).map_err(|e| ActivateError { source: ActivateErrorKind::Io(e) })?;
    let mut content = String::new();
    file.read_to_string(&mut content).map_err(|e| ActivateError { source: ActivateErrorKind::Io(e) })?;
    Ok((content, file))
}
...
fn activate_rprofile(rprofile_content: String, mut file: File) -> Result<(), ActivateError> {
    ...
    file.write_all(content.as_bytes()).map_err(|e| ActivateError { source: ActivateErrorKind::Io(e) })?;
    Ok(())
}
